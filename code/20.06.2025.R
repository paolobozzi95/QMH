...
iv <- feols(share_votes_DC ~ share_civil  | year, data = df)
summary(iv)

################################################################################
model_IV <- feols(tax_burden ~ 1  + factor(year) | share_votes_DC ~ share_civil,
                  cluster ~ province,
                 data = df)
summary(model_IV)
...
msummary(
  ...
  coef_map = c(
    "share_civil" = "Share of civil marriages",
